...
class Game {
    ...
    next_turn() {
        ...
        let turn = this._turn;
        Game.io.in(this.room).emit('game:turn', { turn });
        console.log('On', turn);
        ...
        if (this._turn >= 3) {
            ...
            if (this.allPlayerBusted()) {
                ...
                Game.io.in(this.room).emit('game:ended', { dealerScore, dealerCards });
                this.isOnPlay = false;
                return true;
            }
            ...
            Game.io.in(this.room).emit('dealer:deal', { 'username': 'Dealer', cards, 'seat': 3 });
            const dealerScore = this.dealerScore;
            const dealerCards = this.dealer;
    ...
    standPlayer(socket) {
        ...
        if (socket.isSplitGame) {
            ...
            Game.io.in(socket.myRoom).emit('player:stand', { username, seat });
            socket.hand.push(card);
            Game.io.in(socket.myRoom).emit('player:deal', { username, card, seat });
            return;
        }
        socket.isEnded = true;
        this.next_turn();
